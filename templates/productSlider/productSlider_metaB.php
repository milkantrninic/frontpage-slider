...
class Fpsl_productSliderMeta
{
    ...
    public function fpsl_sd_register_metaboxes()
    {
        ...
        if ($this->selectSlideTheme == "productSlider") {
...
?>
            <p>Add this code to theme file to run slider:</br> &lt;?php echo do_shortcode('[productSlider pagenumber="<?php echo esc_attr($get_popup_post_number); ?>"]'); ?&gt;</p>
...
<label>Izaberite kategoriju koju ce prikazati</label>
    <?php 
    $orderby = 'name';
    $order = 'asc';
    $hide_empty = true ;
    ...
    $getSavedCat = esc_attr(get_post_meta(get_the_ID(), 'selectCat', true));
...
<select name="selectCat">
            ...
            <option value="<?php echo $category->name?>" <?php echo $getSavedCat ==  $category->name ? "selected='true'"  : ""; ?>><?php echo $category->name; ?></option>
            <?php } unset($key, $category );?>
...
<br />
<p class="meta-options hcf_field">
<?php $getSide = esc_attr(get_post_meta(get_the_ID(), "leftRight", true)); ?>
                <label for="<?php echo "leftRight"; ?>">Prikazi kategoriju lijevo ili desno</label>
                <select name="leftRight">
                    <option value="desno"  <?php echo $getSide !==  "lijevo" ? "selected='true'" : "" ; ?>>Desno</option>
                    <option value="lijevo"  <?php echo $getSide == "lijevo" ? "selected='true'"  : "" ; ?>>Lijevo</option>
                </select>
            </p>
            <br />
<p class="meta-options hcf_field">
                <label for="<?php echo "preporucujemo"; ?>">Naslov preporucenih proizvoda</label>
                <input type="text" id="<?php echo "preporucujemo"; ?>" name="<?php echo "preporucujemo"; ?>" value="<?php echo esc_attr(get_post_meta(get_the_ID(), "preporucujemo", true)); ?>">

            </p>
            <br />
<label >Izaberite proizvode koje hocete da preporucite</label>
<div class="wrapboxes">
...
$selected_products = get_post_meta( get_the_ID(), 'selected_products', true );
     ...
     <form id="productSelectionForm" method="post" enctype="multipart/form-data">
        ...
        while ( $products_query->have_posts() ) : $products_query->the_post();
            ...
            <div class="custom-product-wrapper">
                <input type="checkbox" class="product-checkbox" name="selected_products[]" value="<?php echo esc_attr( $product_id ); ?>" <?php checked( in_array( $product_id, $selected_products ), true ); ?> />
